...
def evaluate(df: pd.DataFrame) -> tuple:
    tn, fp, fn, tp = confusion_matrix(y_true=df["compilable_label"], y_pred=df["prediction"]).ravel()
    confusion_matrix_tb = {"tn": tn, "fp": fp, "fn": fn, "tp": tp}

    accuracy = accuracy_score(y_true=df["compilable_label"], y_pred=df["prediction"])
    precision = precision_score(y_true=df["compilable_label"], y_pred=df["prediction"])
    recall = recall_score(y_true=df["compilable_label"], y_pred=df["prediction"])
    f1 = f1_score(y_true=df["compilable_label"], y_pred=df["prediction"])
