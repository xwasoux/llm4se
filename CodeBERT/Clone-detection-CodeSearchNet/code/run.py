...
def decrease_indent(text: str) -> str:
...
def convert_idx_to_input_example(args: argparse.Namespace, tokenizer: AutoTokenizer, index_data: pd.DataFrame, context_data: pd.DataFrame, max_examples_size: int = None) -> datasets.Dataset:
    ...
    if max_examples_size is not None:
        positive_df = context_data[context_data["clone"] == 1]
        negative_df = context_data[context_data["clone"] == 0]
    ...
    for _, row in tqdm(context_data.iterrows(), total=len(context_data)):
        idx1 = row["idx1"]
        idx2 = row["idx2"]
        text1 = decrease_indent(index_data.loc[idx1]["func"])
        text2 = decrease_indent(index_data.loc[idx2]["func"])
        text = text1 + tokenizer.sep_token + tokenizer.cls_token + text2
        clone_label = row["clone"]

        input_examples.append([id, text, clone_label])
        id += 1
...
def load_and_cache_examples(args: argparse.Namespace, tokenizer: AutoTokenizer, eval: bool = False, test: bool = False, max_examples_size: int = None) -> datasets.Dataset:
    ...
    context_data = pd.read_csv(file_path, sep="\t", header=None, names=["idx1", "idx2", "clone"])
...
def main() -> None:
    ...
    if args.do_test:
        ...
        context_data = pd.read_csv(args.test_data_file, sep="\t", header=None, names=["idx1", "idx2", "clone"])
        index_data = pd.read_json(args.index_data_file, lines=True, orient="records", encoding="utf-8").set_index("idx")
        ...
        logger.info("       Positive examples = {}".format(len(context_data[context_data["clone"] == 1])))
        logger.info("       Negative examples = {}".format(len(context_data[context_data["clone"] == 0])))
        logger.info("   Model save path: {}".format(model_save_path))
        ...
        for _, row in tqdm(context_data.iterrows(), total=len(context_data)):
            ...
            prediction_table.append([idx1, idx2, text1, text2, clone_label, prediction])
        prediction_df = pd.DataFrame(prediction_table, columns=["idx1", "idx2", "text1", "text2", "clone_label", "prediction"])
