...
def load_and_cache_examples(args: argparse, eval: bool = False, test: bool = False, max_examples_size: int = None) -> list:
    ...
    pair_data = pd.read_csv(file_path, sep="\t", header=None, names=["idx1", "idx2", "label", "edit_distance", "cosine_label"])
...
def main() -> None:
    ...
    if args.do_test:
        ...
        pair_data = pd.read_csv(args.test_data_file, sep="\t", header=None, names=["idx1", "idx2", "label", "edit_distance", "cosine_label"])
        index_data = pd.read_json(args.index_data_file, lines=True, orient="records", encoding="utf-8").set_index("idx")
