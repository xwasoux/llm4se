...
def main() -> None:
    ...
    if args.do_test:
        ...
        for _, row in tqdm(pair_data.iterrows(), total=len(pair_data)):
            ...
            predict_results.append([text1, text2, label, edit_script, cosine_label, cosine_score])
        result = pd.DataFrame(predict_results, columns=["text1", "text2", "label", "edit_script", "cosine_label", "cosine_score"])
