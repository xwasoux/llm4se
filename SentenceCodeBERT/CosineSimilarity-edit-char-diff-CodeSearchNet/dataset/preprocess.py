...
def distance_to_reciprocal(distance: int) -> float:
    return 1 / (1 + distance)
...
def do_preprocess(args: argparse.Namespace, eval: bool = False, test: bool = False) -> pd.DataFrame:
    ...
    with Pool(num_processes) as pool:
        ...
        results = pool.map(create_edit_code_pair, args_func_list)

    # Combine results from parallel processes
    edit_pair_dfs, edit_func_dfs = zip(*results)
    edit_pair_df = pd.concat(edit_pair_dfs, ignore_index=True)
    edit_func_df = pd.concat(edit_func_dfs, ignore_index=True)

    edit_pair_df.to_csv(output_file, sep="\t", index=False)
    return edit_func_df
