...
def convert_idx_to_input_example(args: argparse.Namespace, index_data: pd.DataFrame, pair_data: pd.DataFrame, max_examples_size: int = None) -> list:
    ...
    if max_examples_size is not None:
        ...
        subtree_pair = pair_data[pair_data["pruning_type"] == "subtree"]
        sequence_pair = sequence_pair.sample(n=max_examples_size//2, random_state=42)
        subtree_pair = subtree_pair.sample(n=max_examples_size//2, random_state=42)
        pair_data = pd.concat([sequence_pair, subtree_pair], ignore_index=True)
    ...
    for _, row in tqdm(pair_data.iterrows(), total=len(pair_data)):
        ...
        reciprocal_label = row["reciprocal_label"]
...
def main() -> None:
    ...
    if args.do_test:
        ...
        for _, row in tqdm(pair_data.iterrows(), total=len(pair_data)):
            ...
            predict_results.append([text1, text2, pruning_type, edit_distance, reciprocal_label, cosine_score])
        result = pd.DataFrame(predict_results, columns=["text1", "text2", "pruning_type", "edit_distance", "reciprocal_label", "cosine_score"])
