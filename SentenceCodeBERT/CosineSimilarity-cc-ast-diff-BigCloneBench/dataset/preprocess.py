...
def convert_edit_script_helper(args):
    index_data, row = args
    idx1 = row['idx1']
    idx2 = row['idx2']
    label = row['label']
    ...
    try:
        edit_script = len(cd.difference(text1, text2, lang="java").edit_script())
    except ValueError:
        edit_script = 0

    cosine_label = distance_to_cosine(edit_script) if label == 1 else -1

    return idx1, idx2, edit_script, cosine_label

def convert_edit_script_parallel(index_data: pd.DataFrame, pair_data: pd.DataFrame) -> pd.DataFrame:
    with Pool() as pool:
        args_list = [(index_data, row) for _, row in pair_data.iterrows()]
        results = list(tqdm(pool.imap(convert_edit_script_helper, args_list), total=len(pair_data)))

    for result, (_, row) in zip(results, pair_data.iterrows()):
        idx1, idx2, edit_script, cosine_label = result
        pair_data.loc[row.name, 'edit_script'] = edit_script
...
def do_preprocess(args: argparse.Namespace, index_data: pd.DataFrame, eval: bool = False, test: bool = False) -> None:
    ...
    df = convert_edit_script_parallel(index_data=index_data, pair_data=df)
